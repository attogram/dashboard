...
@test "github module (plain)" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" =~ "GitHub Repositories for attogram" ]]
  [[ "$output" =~ "base:" ]]
  [[ "$output" =~ "Stars: " ]]
  [[ "$output" =~ "2048-lite:" ]]
}
...
@test "github module (pretty)" {
  ...
  [[ "$clean_output" =~ "GitHub Repositories for attogram" ]]
  [[ "$clean_output" =~ "base:" ]]
  [[ "$clean_output" =~ "Stars: " ]]
...
@test "github module (json)" {
  ...
  [ "$status" -eq 0 ]
  echo "$output" | grep -q -E '"github":{"user":"attogram","url":"https://github.com/attogram","repositories":{'
...
@test "github module (xml)" {
  ...
  [ "$status" -eq 0 ]
  echo "$output" | grep -q -E '<github user="attogram" url="https://github.com/attogram">'
  echo "$output" | grep -q -E '<base>'
  echo "$output" | grep -q -E '<stars>'
...
@test "github module (html)" {
  ...
  [ "$status" -eq 0 ]
  echo "$output" | grep -q -E '<h2><a href="https://github.com/attogram">GitHub Repositories for attogram</a></h2>'
...
@test "github module (markdown)" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" =~ "### [GitHub Repositories for attogram](https://github.com/attogram)" ]]
  [[ "$output" =~ "#### base" ]]
  [[ "$output" =~ "- Stars: " ]]
  [[ "$output" =~ "#### 2048-lite" ]]
}
